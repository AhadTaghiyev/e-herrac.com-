...
@section('content')
    <section class="content">
        <div class="container-fluid">
            <form action="{{ route('admin.advertisements.store') }}" method="post" autocomplete="off" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-9">
                        <!-- Default box -->
                        <div class="card card-primary card-outline">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea data-editor="ckeditor" class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes">{{ old('notes') }}</textarea>
                                    @error('notes')
                                ...
                                <div class="form-group">
                                    <label style="width:100%;text-align:center;" for="images">Images</label>
                                        <input class="form-control @error('images') is-invalid @enderror" type="file" data-fileuploader="multiple" name="images">
                                    @error('images')
                                    ...
                                    <div class="col-md-12">
                                                ...
                                                <div class="input-group-append">
                                                    <select name="currency" data-placeholder="Select currency" id="currency" class="form-control @error('currency') is-invalid @enderror">
                                                        @foreach(\App\Models\Advertisement::$currencies as $key => $value)
                                                            <option value="{{$key}}" @if(old('currency') === (string)$key) selected @endif>{{$value}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <div class="form-group">
                                            <div class="custom-control custom-switch">
                                                <input value="1" name="is_featured" type="checkbox" class="custom-control-input" id="is_featured" @if(old('is_featured')){{'checked'}}@endif>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="auction_id" data-placeholder="Select auction" id="auction_id" class="form-control @error('auction_id') is-invalid @enderror">
                                                ...
                                                @foreach ($auctions as $auction)
                                                    <option value="{{$auction->id}}" @if(old('auction_id') === (string)$auction->id) selected @endif>{{$auction->date->format('d.m.Y')}}</option>
                                                @endforeach
                                            </select>
                                            @error('auction_id')
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="region_id" data-placeholder="Select region" id="region_id" class="form-control @error('region_id') is-invalid @enderror">
                                                ...
                                                @foreach ($regions as $region)
                                                    <option value="{{$region->id}}" @if(old('region_id') === (string)$region->id) selected @endif>{{$region->name}}</option>
                                                @endforeach
                                            </select>
                                            @error('region_id')
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="category_id" id="category_id" data-placeholder="Select category" class="form-control @error('category_id') is-invalid @enderror">
                                                <option value=""></option>
                                                {!! $traverse($categories, '', old('category_id')) !!}
                                            </select>
                                            @error('category_id')
